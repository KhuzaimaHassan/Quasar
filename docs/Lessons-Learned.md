...
**What I'd do differently**: Found that github_token is persistently stored in plain view inside LangGraph checkpoints tables because it is part of AgentState, and we had no application-layer check for repo push access before starting an expensive agent run. We fixed this by:
1. Stripping `github_token` from `AgentState` entirely, relying on passing it solely through the `Command(resume=...)` payload during the interrupt callback so that LangGraph's `PostgresSaver` physically cannot serialize it.
2. Adding a pre-flight `check_repo_write_access` in FastAPI, called by Next.js *before* the LangGraph pipeline starts. This guarantees the user actually has `push` permissions for `targetRepo`, blocking unauthorized commits instantly via a 403 Forbidden instead of waiting for the agent to fail at the end of the run.
3. Fixing a "fail-open" vulnerability in the Next.js `POST /api/agents/run` route handler. Previously, if a user requested GitHub execution but their account lacked a token, the route silently skipped validation instead of rejecting it. We strictly enforced a `400 Bad Request` rejection for missing tokens, verifying its functionality with E2E Next.js fetch tests against real accounts.
